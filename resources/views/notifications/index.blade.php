...
$types = ['devis_envoye'=>0,'devis_recu'=>0,'devis_accepte'=>0,'devis_refuse'=>0,'commande_creee'=>0,'facture_generee'=>0,'stock_faible'=>0];
...
<div class="notif-grid">
    ...
    <div style="display:flex;flex-direction:column;gap:10px;">
        ...
        $icons = [
            ...
            'devis_recu'      => ['bg'=>'rgba(168,85,247,.14)','stroke'=>'#c084fc','path'=>'M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z'],
            'devis_accepte'   => ['bg'=>'rgba(34,197,94,.14)','stroke'=>'#4ade80','path'=>'M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z'],
            'devis_refuse'    => ['bg'=>'rgba(239,68,68,.14)','stroke'=>'#f87171','path'=>'M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2m7-2a9 9 0 11-18 0 9 9 0 0118 0z'],
            ...
            'demande_devis'   => ['bg'=>'rgba(245,158,11,.14)','stroke'=>'#fbbf24','path'=>'M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z'],
            'message_recu'    => ['bg'=>'rgba(14,165,233,.14)','stroke'=>'#38bdf8','path'=>'M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z'],
            'demande_acceptee'=> ['bg'=>'rgba(34,197,94,.14)','stroke'=>'#4ade80','path'=>'M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z'],
            'demande_refusee' => ['bg'=>'rgba(239,68,68,.14)','stroke'=>'#f87171','path'=>'M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2m7-2a9 9 0 11-18 0 9 9 0 0118 0z'],
        ];
            ...
            <div style="flex:1;min-width:0;">
                <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:12px;">
                    <div style="flex:1;">
                        ...
                        {{-- Actions pour demande_devis / message_recu → ouvrir la conversation --}}
                        @if(in_array($notif->type, ['demande_devis','message_recu','demande_acceptee','demande_refusee']) && isset($notif->data['demande_id']))
                        <div style="margin-top:10px;">
                            <a href="{{ route('demandes.conversation', $notif->data['demande_id']) }}"
                               style="display:inline-flex;align-items:center;gap:6px;padding:7px 16px;background:rgba(245,158,11,.12);border:1px solid rgba(245,158,11,.3);border-radius:9px;color:#f59e0b;font-size:12px;font-weight:700;text-decoration:none;transition:all .15s;"
                               onmouseover="this.style.background='rgba(245,158,11,.2)'" onmouseout="this.style.background='rgba(245,158,11,.12)'">
                        ...
                        {{-- Actions pour devis_recu --}}
                        @if($notif->type === 'devis_recu' && isset($notif->data['devis_id']))
                        @php $devisId = $notif->data['devis_id']; @endphp
                        <div style="display:flex;align-items:center;gap:8px;margin-top:10px;flex-wrap:wrap;">
                            <a href="{{ route('devis.client', $devisId) }}"
                               style="display:inline-flex;align-items:center;gap:5px;padding:6px 14px;background:rgba(168,85,247,.12);border:1px solid rgba(168,85,247,.3);border-radius:8px;color:#c084fc;font-size:12px;font-weight:700;text-decoration:none;">
                                <svg width="12" height="12" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                                Voir le devis
                            </a>
                            <form method="POST" action="{{ route('devis.refuser-client', $devisId) }}" style="display:inline;">
                                @csrf @method('PATCH')
                                <button type="submit"
                                    style="display:inline-flex;align-items:center;gap:4px;padding:6px 12px;background:rgba(239,68,68,.1);border:1px solid rgba(239,68,68,.25);border-radius:8px;color:#f87171;font-size:12px;font-weight:700;cursor:pointer;font-family:inherit;"
                                    onclick="return confirm('Refuser ce devis ?')">
                                    Refuser
                                </button>
                            </form>
                            <form method="POST" action="{{ route('devis.accepter-client', $devisId) }}" style="display:inline;">
                                @csrf @method('PATCH')
                                <button type="submit"
                                    style="display:inline-flex;align-items:center;gap:4px;padding:6px 12px;background:rgba(34,197,94,.15);border:1px solid rgba(34,197,94,.3);border-radius:8px;color:#4ade80;font-size:12px;font-weight:700;cursor:pointer;font-family:inherit;"
    ...
    <div style="display:flex;flex-direction:column;gap:14px;">
        <div class="stat-mini">
            ...
            <div class="sm-row">
                <span style="color:var(--muted);">Devis reçus</span>
                <span style="font-weight:600;color:#c084fc;">{{ $types['devis_recu'] }}</span>
            </div>
            <div class="sm-row">
